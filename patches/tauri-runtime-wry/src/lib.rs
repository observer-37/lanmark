...
fn handle_event_loop<T: UserEvent>(
  ...
  match event {
    ...
    e @ Event::Resumed | e @ Event::Suspended => {
      // Lanmark patch (tauri#15671): surface mobile resume/suspend to the app
      // even when there are zero windows — the loop body below only runs
      // per-window dispatch, so with no windows the resume was silently
      // dropped and a FGS-kept-alive process could never rebuild its webview.
