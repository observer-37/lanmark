...
6. **wikilink**：打开「生活/读书笔记」末尾的 `[[会议纪要-产品周会]]`——在 Lanmark 的 WYSIWYG 里显示为 `⟦会议纪要-产品周会⟧`，编辑保存后用 Obsidian 打开应仍是活链接
7. **Obsidian**：用 Obsidian 打开同一个目录，确认笔记/图片/结构正常
...
关联笔记：[[会议纪要-产品周会]]（wikilink，Lanmark 编辑保存后在 Obsidian 里应仍是活链接）。
EOF
